        ...
        match &self.item {
            Value::Row(dict) => match &member.item {
                RawPathMember::String(key) => Ok({
                    ...
                    Value::Row(dict).tagged(&self.tag)
                }),
            ...
            Value::Table(array) => match &member.item {
                ...
                RawPathMember::Int(int) => {
                    let mut array: Vec<Tagged<Value>> = array.clone();
                    ...
                    insert_data_at_index(&mut array, int.tagged(member.span), new_value.clone())?;
                    Ok(Value::Table(array).tagged(&self.tag))
                }
            },
    ...
    pub fn insert_data_at_column_path(
        ...
        match split_path.split_last() {
            ...
            Some((last, front)) => {
                ...
                for member in front {
                    current = current
                        .get_data_by_member(member)
                        .expect("FIXME: unimplemented")
                        .clone()
                }
